...
- `/etc/nginx/cookflow.htpasswd`：私有厨房的访问凭据，不进入 Git。
...
应用使用独立的 cookflow 系统用户，只监听回环地址 8100，由 Nginx 的 HTTPS 和访问密码保护。目前是一个私有厨房，不是多租户服务。

更新代码时使用新的发布目录，更换 current 链接后重启 cookflow。更新食谱时保留旧文件备份，原子替换 catalog.sqlite；不要覆盖 user.sqlite。回滚代码只需恢复 current 指向上一发布目录。

访问密码通过服务器或管理员的私有渠道获取，不写入公开仓库。自动构建已配置；生产发布目前由管理员通过 SSH 执行。
